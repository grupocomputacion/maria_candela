...
# ---------------------------------------------------------
# 2. RECETAS (LÓGICA 100% ESPEJO - SIN PANDAS EN EL JOIN)
# ---------------------------------------------------------
    ...
    if finales:
        ...
        with col_izq:
            ...
            # v_data contiene: (id, nombre, margen1, margen2)
            v_data = [f for f in finales if f[1] == v_sel][0]
            id_v_final = v_data[0]
            
            with st.form("add_insumo_web"):
                st.write("### Vincular Materia Prima")
                i_sel = st.selectbox("Insumo", [i[1] for i in insumos]) if insumos else st.error("No hay insumos")
