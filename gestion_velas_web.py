...
# ---------------------------------------------------------
# 2. RECETAS (SOLUCIÓN DEFINITIVA SQL)
# ---------------------------------------------------------
    ...
    if finales:
        ...
        with col_der:
            ...
            # Blindaje total de la query
            query_rec = "SELECT recetas.id, productos.nombre, recetas.cantidad, productos.costo_u, (recetas.cantidad * productos.costo_u) FROM recetas JOIN productos ON recetas.id_insumo = productos.id WHERE recetas.id_final = ?"
            ...
            if rows:
                ...
                if st.button("💾 GUARDAR PRECIOS"):
                    conn.execute("UPDATE productos SET precio_v=?, precio_v2=?, margen1=?, margen2=?, costo_u=? WHERE id=?", (p1, p2, m1, m2, costo_b, id_v_f))
...
# ---------------------------------------------------------
    ...
    with st.form("form_f4"):
        ...
        i_cant = st.number_input("Cantidad Comprada", min_value=0.01)
...
# ---------------------------------------------------------
# 5. CAJA Y FILTROS (FUNCIÓN 5)
# ---------------------------------------------------------
elif menu == "📊 Caja y Filtros (F5)":
    ...
    if not df_filt.empty:
        st.metric("SALDO NETO", f"$ {df_filt['Monto'].sum():,.2f}")
